...
"""
    ...
    def run_phase_dialogue(self, engine: Any, phase: str, active_faction: str, objective: str):
        ...
        for u in engine.wsm.get_state().units.values():
            ...
            weap_info = ""
            if "Combat" in phase and hasattr(u, 'entity_profile'):
                weaps = [f"{w.get('Name','')} (Range: {w.get('Range','N/A')})" for w in u.entity_profile.get('collections',{}).get('Weapons',[])]
        ...
        if final_action:
            ...
            elif "Combat" in phase:
                resolve_func = self.get_or_generate_mechanic("resolve_combat", "Resolve combat firing from an attacker to a target.")
                if resolve_func:
                    matches = re.finditer(r'\[FIRE:\s*(.*?)\]', final_action, re.IGNORECASE)
                    for match in matches:
                        cmd_str = match.group(1).strip()
                        unit = next((u for u in engine.wsm.get_state().units.values() if u.faction == active_faction), None)
                        target_match = re.search(r'Target=([^,\]]+)', cmd_str, re.IGNORECASE)
                        if unit and target_match:
                            target_id = target_match.group(1).strip()
                            target = engine.wsm.get_state().units.get(target_id)
                            if target:
                                try:
                                    res = resolve_func(unit, target, engine.wsm, f"[FIRE: {cmd_str}]")
                                    print(f"[MASTER ARBITER]: Combat Resolved: {res}")
                                except Exception as e:
                                    print(f"[MASTER ARBITER]: Error resolving combat for {unit.id} vs {target.id}: {e}")
                            else:
                                print(f"[MASTER ARBITER]: Target {target_id} not found.")
        else:
